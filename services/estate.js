...
const getProvinces = () =>
  new Promise((resolve, reject) => {
    localConnection.query(
      "select * from province order by description asc;",
      (err, rows, fields) => {
        if (err) {
          reject(err);
          return;
...
const getMunicipalities = (provinceId) =>
  new Promise((resolve, reject) => {
    let query = `select * from municipality_district where id like ${localConnection.escape(
      provinceId + "%"
...
const getSectors = (municipalityId) =>
  new Promise((resolve, reject) => {
    let query = `select * from sector where id like ${localConnection.escape(
      municipalityId + "%"
    )} order by description asc;`;
    localConnection.query(query, (err, rows, fields) => {
      if (err) {
...
const getAmenities = (estateId = "") =>
  new Promise((resolve, reject) => {
    let query = `select a1.*,IF(s.id is null,0,1) as active
      ...
      on a.id = ae.amenity_id where ae.estate_id = ${localConnection.escape(
        estateId
      )}) s
      on a1.id = s.id;`;
...
const updateAmenities = (estateId, amenities) =>
  new Promise((resolve, reject) => {
    let deleteQuery = `delete from amenity_vs_estate where estate_id = ${localConnection.escape(
      estateId
    )}`;
    localConnection.query(deleteQuery, (error, rows, fields) => {
      ...
      for (let i = 0; i < amenities.length; i++) {
        ...
        updateQuery += `(${localConnection.escape(id)},${localConnection.escape(
          estateId
        )})`;
...
const saveGallery = async (estateId, gallery) => {
  for (let i = 0; i < gallery.length; i++) {
    ...
    if (imageString != "" && imageString != undefined) {
      await localQuery(
        `insert into gallery values (default,'${name}', ${estateId})`
      );
    }
  }
};
